...
Description:
	...
	BAM file with MD tags (fast) as generated by 'samtools calmd'. Intended for
...
eg. ./change_sam_queries.py -f reference.fa -G -T 1 -t /var/tmp input.bam output.bam
...
##################
## DEFINE __MAIN__
def main(BAM,OUT,GENOMIC,THREADS=1,TEMP=None,FASTA=None):
	...
	with pysam.AlignmentFile(BAM, "rb") as original:
		...
		for ref in original.get_index_statistics():
			...
			if ref.mapped > 0:
				...
				job = pool.apply_async(worker, (BAM, TDIR, header, ref.contig, reference, GENOMIC))
				jobs.append(job)
	...
	# output the final file + clean-up
...
def validate_MD(qseq,MD,CT,count):
	...
	if (("T" in qseq) and (not "C" in MD) and CT) or (("A" in qseq) and (not "G" in MD) and not CT): count += 1
	return count
...
def worker(BAM,TDIR,header,ref,reference,GENOMIC):
...
parser.add_argument('-t','--temp', metavar='<TEMP>', help='[OPTIONAL] Path to temp directory. [default: /var/tmp]')
...
if __name__ == '__main__':
	main(args.inbam,args.outbam,args.GENOMIC,args.THREADS,args.temp,args.fasta)
